...
def _get_system_proxy() -> str | None:
    """Read system/IE proxy settings (Windows registry or env vars)."""
    proxies = urllib.request.getproxies()
    return proxies.get("https") or proxies.get("http") or None


def _create_http_transport() -> httpx.HTTPTransport:
    """Create HTTP transport, applying system proxy when available."""
...
def _create_retry_transport(
    ...
    return RetryTransport(retry=retry, transport=_create_http_transport())
...
def reset_client() -> None:
    """Reset the module-level client (e.g. after code reload or proxy change)."""
    _close_client()
